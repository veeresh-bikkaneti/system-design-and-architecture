...
- **Tool calling, LangChain-shaped.** The page, not the model, runs `search_lessons` and `read_concept`. Tool output is data, not new instructions. When no lesson covers a general tech question, the model answers from its own trained knowledge instead, clearly labelled "General knowledge overview" rather than shown as a lesson citation. If the model's draft is unusable, the grounded reply stays.

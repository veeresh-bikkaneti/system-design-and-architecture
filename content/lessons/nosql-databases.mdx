...
<VideoCard videoId="4YR24Bs1aY0" title="NoSQL Databases Explained: The 4 Types You Must Know" source="TechingEasy" description="Quick tour of all four NoSQL types and when they beat relational — the motivation in one video." />
...
<VideoCard videoId="vosF_JpTV2I" title="4 Types of NoSQL Databases Explained | System Design Interview #9" source="Learn with Manoj" description="System-design walkthrough of the four NoSQL shelves with concrete examples for each." />
...
<VideoCard videoId="Dwt8R0KPu7k" title="How Key value Stores Work (Redis, DynamoDB, Memcached)?" source="ByteByteGo" description="Builds a scalable key-value store from scratch — labels, lookups, and speed, demystified." />
...
<VsToggle
  ...
  b={{
    ...
    note: "More flexible, but reads now need two lookups instead of one.",
  }}
/>
...
<VideoCard videoId="GV9VBwH_h1U" title="MongoDB Explained in 10 Minutes | SQL vs NoSQL | Jumpstart" source="MongoDB" description="Official MongoDB ten-minute intro to documents, flexible schemas, and SQL contrasts." />
...
<VideoCard videoId="J-cSy5MeMOA" title="Apache Cassandra Database – Full Course for Beginners" source="freeCodeCamp.org" description="freeCodeCamp's full Cassandra course — data modeling for the giant distributed ledger." />
...
<VideoCard videoId="wO5AI1oDGjQ" title="Graph Databases in 60 seconds" source="Neo4j" description="Neo4j's official sixty-second explainer on connecting data with graph relationships." />
...
<VideoCard videoId="xh4gy1lbL2k" title="NoSQL Database Tutorial – Full Course for Beginners" source="freeCodeCamp.org" description="Full beginner course tying all four models together so the differences stick." />
...
One term from that row deserves a plain-English unpacking: **ACID** is four promises bundled
into every transaction — **a**tomicity (all-or-nothing), **c**onsistency (your data's rules
always hold), **i**solation (concurrent transactions don't trip over each other),
**d**urability (committed means it survives a crash) — the reason money lives in the cabinet.
...
<VideoCard videoId="zoiGdEDzg0A" title="SQL vs NoSQL Explained | Choosing the Right Database in System Design" source="Bikki Mahato" description="Tables vs. documents, joins vs. denormalization, ACID vs. BASE — the honest trade-offs so you can pick a database like an engineer, not a fan." />
...
<VideoCard videoId="BHqjEjzAicA" title="CAP Theorem Simplified" source="ByteByteGo" description="Consistency, availability, partition tolerance — pick two: Alex Xu&#x27;s crisp tour of which guarantees each NoSQL bin chose and what it gave up." />
...
- **Bank ledgers, inventory counts, anything where money must never be wrong** →
  **Stay with the filing cabinet.** ACID transactions exist for a reason, and "eventually
  consistent" — every bin agreeing *eventually*, not right now — is a terrifying phrase
  next to someone's paycheck.
...
<VideoCard videoId="FzlpwoeSrE0" title="NoSQL vs SQL – Which Type of Database Should You Use?" source="freeCodeCamp.org" description="Practical guide to choosing SQL or NoSQL based on what your app actually needs." />

## Takeaways

1. **NoSQL is often read as "not only SQL,"** not "no SQL ever" — a popular backronym (the
   1998 original just meant "non-SQL"). It's four families of flexible
...
- [Redis Course – In-Memory Database Tutorial](https://www.youtube.com/watch?v=XCsS_NVAa1g) — freeCodeCamp.org (~1.5h). Data structures, transactions, Pub/Sub, and Lua scripting — the key-value and document side of NoSQL in depth.
- [System Design Course – APIs, Databases, Caching, CDNs, Load Balancing & Production Infra](https://www.youtube.com/watch?v=C842vFY5kRo) — freeCodeCamp.org (~2h 05m). The SQL vs NoSQL vs graph chapters — a decision framework for picking the right database.

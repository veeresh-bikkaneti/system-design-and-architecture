...
<VideoCard videoId="jTe6Q78uzQA" title="Database Optimization in System Design | Replication, CAP Theorem, Partitioning & Sharding Explained" source="Web Detailed by Mohi" description="Explains why databases become bottlenecks and how partitioning and sharding unlock performance at scale." />
...
<VideoCard videoId="Ws2vMYZgN84" title="Sharding, Partitioning & Indexing in Database| System Design | Episode 09" source="The Tech Intern" description="Builds intuition with everyday pictures like a phone book at two counters, then walks range vs hash sharding." />
...
- **Directory-based** — a central card catalog maps each book to its branch. Moving books
  ...
  burns down, nobody can find anything. Real catalogs don't rely on a single copy, though:
  MongoDB's config servers run as a replica set, and ZooKeeper- or etcd-backed directories
  answer only by quorum. The catalog is a critical dependency that must itself be made
  highly available — replicate it like everything else, and the fire becomes a story, not
  an outage.
- **Consistent hashing** — arrange branches and books on a ring so that adding or removing a
  ...
  The full mechanics — virtual nodes, the ~1/N resize math — live in the sharding-strategies
  lesson; here you just need the ring idea and why it beats modulo hashing on resize.
...
<VideoCard videoId="ds-v2gIvTpo" title="Why One Database Is Never Enough — Partitioning Deep Dive (DDIA Ch. 6)" source="Savant Space" description="Compares key-range vs hash partitioning, hot spots, and consistent hashing for rebalancing, straight from DDIA." />
...
<VideoCard videoId="rWcIVJ1asWc" title="System Design | Sharding | System Design: How Distributed Databases Scale (Shards, Joins, &amp; Skew)" source="CS AI Think Tinker Learn (unverified — confirm channel on YouTube)" description="Split one recipe book across four kitchens and every dinner becomes a phone tree — that&#x27;s the cross-shard tax on joins, aggregations, and scatter-gather." />
...
When the constraints start fighting each other — a key with high cardinality that scatters
related rows, a whale tenant that dwarfs everyone else — the sharding-strategies lesson
picks up this thread with strategy selection under pressure: virtual nodes, salted keys,
directory surgery for hot tenants, and the resharding cost math.

<VideoCard videoId="fQc2AkDiPXE" title="What Is a Partition Key? Routing, Pruning, Hot Partitions, and How to Choose One" source="Kandi Brian" description="Walks through hash, range, and composite keys, hot partitions, and how cardinality shapes a good choice." />
  ...
  questions={[
    {
      ...
      options: ["Directory-based", "Range-based", "None of the above", "Hash-based"],
      correctIndex: 3,
    },
    {
...
- [System Design Concepts Course and Interview Prep](https://www.youtube.com/watch?v=F2FmTdLtb_4) — freeCodeCamp.org (~54m). Sharding, replication, and ACID chapters in one concise course.

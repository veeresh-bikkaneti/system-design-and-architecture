...
<VideoCard videoId="DUxHZVBpJWE" title="Chapter 11: Stream Processing Explained" source="deferstech" description="The DDIA chapter on stream processing, distilled: Kafka&#x27;s log-first design, CDC, event time vs processing time, windows, stream joins, and exactly-once — the missing manual for the live ticker." />
...
<VideoCard videoId="fU9hR3kiOK0" title="Turning the database inside out with Apache Samza" source="Martin Kleppmann (Strange Loop)" description="The DDIA author himself: treat data as a stream of immutable facts, and a live counter becomes just a rollup of the click log — the core mental model behind real-time aggregation." />
...
<VideoCard videoId="lW68_KeMNnA" title="Your Stream Is Falling Behind: Backpressure, Lag, and Batching" source="SwetankLabs" description="What actually pages you at 2 a.m.: consumer lag, backpressure, skewed partitions, and buffer pileups — like a traffic jam on the ticker — plus how much spare capacity you need to drain it." />
...
- [Ad Metrics Collection System Design | Kafka, Flink, ClickHouse, Redis | Ad Click Aggregation](https://www.youtube.com/watch?v=tFv2ydbD0Ys) — PartTimeEngineer. AI-narrated alternative.

...
<VideoCard videoId="hWRRdICvMNs" title="What is an API Gateway?" source="IBM Technology" description="IBM's primer shows where a gateway earns its keep in a microservices setup." />
...
<VideoCard videoId="hQLvgI82MZg" title="What is an API Gateway" source="Dargslan" description="Lines up gateways, reverse proxies, load balancers, and BFFs side by side." />
...
<VideoCard videoId="qwASxvKf10k" title="The API Gateway Pattern: When Your Microservices Need a Traffic Cop" source="System Design Lab" description="Covers routing, auth, rate limiting, and aggregation — the gateway's daily jobs." />
...
<VideoCard videoId="t7RCQ_t6A7k" title="Communication Patterns Between Microservices [Free Meetup]" source="life michael" description="Places BFF among gateway-era patterns, so you see exactly where it fits." />
...
<VideoCard videoId="EMmxNnq_T50" title="The API Gateway - Software Architecture" source="The Software Mentor" description="Weighs when a gateway helps against its cost and single-point-of-failure risk." />
...
- [API Gateway Pattern and Kong in Microservices](https://www.classcentral.com/course/youtube-api-gateway-pattern-kong-in-a-microservices-world-221565) — Marco Palladino (Kong CTO), Linux Foundation conference (~42 min). Routing, security, and rate limiting through a real gateway.
- [Mastering Microservices with Kong](https://www.classcentral.com/course/youtube-mastering-microservices-with-kong-by-maarten-mulders-196097) — Maarten Mulders, Devoxx (~49 min). Security, plugins, and monitoring in a working gateway setup.

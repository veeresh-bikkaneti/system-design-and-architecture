...
- **A ledger entry is an event.** `Deposited(500)`, `Withdrew(200)` — past tense,
  immutable, a fact about something that already happened. (The DDD lesson's logbook,
...
disagree and nobody knows. The DDD lesson's answer stands: the **transactional
outbox** — the event lands in an outbox table inside the same transaction as the state
change, and a relay publishes it afterward. One transaction, no distributed commit, no

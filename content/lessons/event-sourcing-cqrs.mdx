...
<VideoCard videoId="xPmQxYIi5fA" title="Event Sourcing Explained Using Football" source="Metaphorically Speaking" description="Uses a football analogy to show why storing facts (a ledger) beats storing only the current score." />
...
<VideoCard videoId="8JKjvY4etTY" title="Event Sourcing • Greg Young • GOTO 2014" source="GOTO Conferences" description="Greg Young contrasts an erasable ledger with an immutable event log versus CRUD's overwrite model." />
...
<VideoCard videoId="VdcYjQV19GM" title="Event Sourcing Explained - Why Immutability Changes Everything" source="José Cruz (IT Architect)" description="Walks through immutability, append-only event stores, and deterministic replay rebuilding state." />
...
<VideoCard videoId="5aznkIEvkKc" title="CQRS & Event Sourcing Code Walk-Through" source="CodeOpinion" description="Code walk-through showing events published to consumers that update projection read models for queries." />
...
<VideoCard videoId="eAIkomEid1Y" title="Event Sourcing: Rehydrating Aggregates with Snapshots" source="CodeOpinion" description="Shows how snapshots checkpoint aggregate state so rebuilds skip replaying the full event stream." />
...
<VideoCard videoId="Spp-uzXzzdE" title="CQRS in Python: Clean Reads, Clean Writes" source="ArjanCodes" description="Refactors an API into separate write and read stores with a projector, and names when CQRS is worth it." />
...
<VideoCard videoId="GzrZworHpIk" title="Event Sourcing   You are doing it wrong by David Schmitz" source="Devoxx" description="Conference talk on real-world event sourcing failures: Kafka-as-event-sourcing, compliance, and maintainability costs." />
...
<VideoCard videoId="t2AI9hODJ2E" title="Event Sourcing in Production - Real-Time CQRS/DDD .NET Apps with EventStore" source="Barcelona .NET Core" description="Covers Reactive Trader Cloud on EventStore, including production problems teams faced and how they solved them." />
...
- [Distribute your microservices data with events, CQRS, and event sourcing](https://www.youtube.com/watch?v=HdvWfr2KwA0) — Edson Yanaga, Red Hat Developer DevNation Tech Talk (~36m). CQRS and event sourcing applied to real distributed data problems with REST, brokers, and Kafka.
- [Event Sourcing – You are doing it wrong](https://www.youtube.com/watch?v=GzrZworHpIk) — David Schmitz, Devoxx 2018 (~43m). Five years of production war stories across finance and insurance — the "diary with consequences" every senior should see.

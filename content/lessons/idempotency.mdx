...
<VideoCard videoId="DqUyk6SXBT4" title="Idempotency on AWS — Explained" source="System Design Lab" description="Joud opens on every backend dev&#x27;s nightmare — the double charge — then shows why retries make it inevitable and how to stop it" />
...
<VideoCard videoId="4OuaONkZw1I" title="Why Idempotency is very critical in Backend Applications" source="Hussein Nasser" description="Hussein Nasser builds the core intuition — repeat the same call, get the same result — the idea behind the elevator button" />
...
<VideoCard videoId="6dVNdFwqeKs" title="Idempotency in Cows and REST APIs" source="Todd Fredrich (RestApiTutorial)" description="A short classic from REST tutorial author Todd Fredrich that nails the plain-English definition: same call, same effect, every time" />
...
<VideoCard videoId="8yp2espaaaQ" title="Idempotency Keys - Redis for Developers" source="The Software Mentor" description="One atomic SET NX claims the slot — like the elevator button lighting up: claimed means the work is already handled, so skip it" />
...
<VideoCard videoId="0cwRRCBpPPM" title="Apache Kafka Idempotency Explained / Prevent Duplicate Messages / Kafka Idempotent Producer" source="Coding with Neeraj" description="Neeraj shows how Kafka&#x27;s idempotent producer tags every message with a producer ID and sequence number, so the broker can tell a fresh send from a retried one" />
...
<VideoCard videoId="weBA-sQibhY" title="Your Background Jobs Are Silently Running Twice" source="The Merge Log" description="Four ways jobs go wrong — lost jobs, double-runs, poison messages, thundering herds — and why idempotency, not exactly-once, is the actual fix" />
...
<VideoCard videoId="hLoIWC2NSNI" title="Why a Timeout Doesn&#x27;t Mean It Failed" source="The Hot Path" description="A timeout means &quot;I don&#x27;t know,&quot; not &quot;no&quot; — so every retry is a bet, and polite retries bring backoff, jitter, and an idempotency key" />
...
<VideoCard videoId="qKoLrvAcuIQ" title="Idempotency in ASP.NET Core (.NET 10): The Definitive Guide for 2026" source="Frontend to Backend with Rohan" description="Rohan lines up GET vs POST vs PUT to show which methods are idempotent by nature — then builds the key pattern in Redis for the ones that aren&#x27;t" />
...
<VideoCard videoId="IP-rGJKSZ3s" title="The Two Generals&#x27; Problem" source="Tom Scott" description="Tom Scott tells the classic story of two generals who can never be sure a message arrived — the same math that pops the exactly-once bubble" />

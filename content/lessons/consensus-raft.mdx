...
{/* ANIMATION: StepThrough showing a 5-node cluster: leader heartbeats, leader crash, randomized election timeouts, and a new leader elected */}
...
- **Election time**: roughly one election timeout (150–300 ms) to notice the leader is
  ...
  via the leader (etcd's default serializable reads go through the leader's quorum check).
...
- Diego Ongaro & John Ousterhout, "In Search of an Understandable Consensus Algorithm (Extended Version)" (USENIX ATC 2014) — the Raft paper itself.

...
<VideoCard videoId="M6UZ7pVD-rQ" title="System design mock interview: 'Design WhatsApp or Telegram' (with ex-Google EM)" source="IGotAnOffer: Engineering" description="Watch a candidate pin down scope and requirements with an ex-Google engineering manager before drawing a single box — scoping under interview pressure." />
...
<VideoCard videoId="vvhC64hQZMk" title="WHATSAPP System Design: Chat Messaging Systems for Interviews" source="Gaurav Sen" description="Gaurav Sen traces one message from send to push — persistent connections, queues, delivery acks — like following a single order through a busy kitchen." />
...
<VideoCard videoId="M1Xv-sxBG1E" title="Design the News Feed (Twitter / X) — System Design Interview" source="The Hot Path" description="Like a group text with five million members: copying one message into every inbox is simple until a celebrity posts — that&#x27;s when fan-out-on-write breaks and the hybrid fix kicks in." />

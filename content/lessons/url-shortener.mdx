...
<PacketFlow
  stages={["Browser", "API", "Key service"]}
  servers={["db-shard-1", "db-shard-2", "db-shard-3"]}
  requestLabel="POST"
/>

The shorten path above: your POST arrives, the API spends a pre-fetched ticket from the
key service, and the row lands on the shard its key hashes to. Now the redirect path,
...
- **The key service goes down.** App servers keep shortening from their local batches —
  ...
  keys at ~100 shortens/sec buys you ~15 minutes; size up from there), alert loudly, and
...
- Bitly Engineering blog — posts on scaling Bitly's URL-shortening infrastructure to billions of requests.

...
<VideoCard videoId="pCvZtjoRq1I" title="Traversy Media MVC" source="Traversy Media" description="Brad builds a tiny MVC framework from scratch — routing, controllers, models, views — so you can see exactly what a framework adds on top of plain code." />
...
<VideoCard videoId="yU6JBscFCVc" title="The Four Pillars of OOP Explained in Python" source="Kandi Brian" description="Covers the four OOP pillars — the kitchen skills you'll reuse when the waiter metaphor arrives." />
...
<VideoCard videoId="A4X2uE_5hDU" title="MVC Design Pattern Demystified" source="Dara Oladapo | Code, Cloud and DevOps" description="Plain-English walkthrough of Model, View, Controller and what each object owns." />
...
<VideoCard videoId="izZ858Gbwh0" title="MVC, MVP and MVVM Explained" source="Vikas Kerni&#x27;s Software Architecture" description="A platform-neutral three-way comparison — how the controller, the presenter, and the view-model each split responsibility between the view and the model." />
...
server retreated to being a very good kitchen: an **API** — a fixed menu of requests the
dining room can place, answered as data.
...
<VideoCard videoId="FQPlEnKav48" title="I built 10 web apps... with 10 different languages" source="Fireship" description="One developer rebuilds the same app in ten languages — watch MVC's ideas survive the remix." />
...
with its own little compartments of state, a tidy public surface (its **props** — the
inputs a parent component hands down, like the order ticket the waiter carries in), and no
...
<VideoCard videoId="x4rFhThSX04" title="Learn React JS - Full Beginner’s Tutorial & Practice Projects" source="freeCodeCamp.org" description="freeCodeCamp beginner course built around components — the foundation of React thinking." />
  ...
  questions={[
    {
      ...
      options: [
        "The kitchen, because the kitchen does all the real work",
        "The menu board, because it displays every option available",
        "The waiter, because it's the only one that takes your input to the kitchen",
        "The customer, because every order starts with them",
      ],
      correctIndex: 2,
    },
    {
      ...
      options: [
        ...
        "Everything — the model renders each view directly on every change",
        "Only the view's address, so the model can push updates to it",
        "It builds the view's HTML itself whenever its state changes",
      ],
      correctIndex: 0,
    },
    {
      ...
      options: [
        "The model and the view merge into a single object that does both",
        "The presenter is removed, and the view talks straight to the database",
        "MVP requires a relational database underneath to work",
        "The view becomes passive — the presenter carries every message both ways",
      ],
      correctIndex: 3,
    },
    {
      ...
      options: [
        "The button edits its sibling components directly, in place",
        "It calls a handler that updates state; new state flows down as props and re-renders the UI",
        "The click travels straight to the database and back again",
        "The view observes the model and redraws itself on change",
      ],
      correctIndex: 1,
    },
    {
      ...
      options: [
        "React apps must be prototyped with physical toys first",
        "React is slower to build with than plain JavaScript",
        "Compose small components instead of inheriting from big ones",

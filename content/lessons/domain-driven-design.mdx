...
things depending on which service you asked. Then it sounds like the cheapest insurance
...
Contexts still talk to each other — just not through shared tables or direct calls. They
exchange **integration events** over async transport. The front desk publishes
...
## Aggregates: the bouncers of your invariants
...
front-desk manager, the housekeeping lead, the billing person who knows where the bodies
are buried — and you cover a wall in sticky notes. No laptops. This is **event
...
Since this is the advanced tier, the failure modes:

- **The polling interval is your latency floor.** Poll every 500 ms and every
  integration event arrives at least half a second stale. Tune the interval, or skip
...
Notice the through-line of the whole lesson: domain events are the vocabulary, the
outbox is the delivery truck, and event sourcing is what happens when you keep every
word ever spoken.
...
- Eric Evans, *Domain-Driven Design: Tackling Complexity in the Heart of Software* (Addison-Wesley, 2003) — the "blue book"; ubiquitous language, bounded contexts, aggregates, and the original domain-event modeling guidance.
...
- software-architektur.tv, [Nicole Rauch zu DDD, Event Storming & Specification by Example](https://software-architektur.tv/2020/09/10/folge017.html) (Folge 17, 2020) — event storming as collaborative DDD modeling; specification by example as building the right thing.

...
<VideoCard videoId="QmX2NPkJTKg" title="Designing INSTAGRAM: System Design of News Feed" source="Gaurav Sen" description="Builds Instagram from ER diagram to celebrity fan-out so every design choice feels earned — like watching the machine get assembled before it&#x27;s switched on." />
...
<VideoCard videoId="K6KUb4ci1hQ" title="Design Requirements: The First Step of Every System Design Interview" source="AlgoJS" description="Covers functional vs non-functional requirements — the scoping questions to ask before designing." />
...
<VideoCard videoId="TEt5hAxv1WA" title="System Design Interview Course • Back of the Envelop Calculation aka Capacity Estimates" source="Anubhav Sethi" description="Drills the numbers worth memorizing and the four estimation formulas, then works them through URL-shortener and Google-Drive examples — times tables for scale." />
...
<VideoCard videoId="ZirDk7ZYE4E" title="Design a Video Streaming Platform (Netflix/YouTube) — System Design Interview" source="Phani Thaticharla" description="Details the upload and transcoding pipeline: resumable uploads, workers, and async processing." />
...
Sharding Strategies lesson, applied. And here you face a real fork in the road, because
the shard key is a trade-off with no painless answer:

**Shard by user ID**, and every photo a person uploads lands on the same slice of the
ledger. "Show me Maya's latest 20 photos" is a single-shard lookup — the system's most
common read stays cheap. The cost is the celebrity: all of their uploads land on one
shard, and every fan's profile view hammers it until that shard runs like a furnace.
Real systems survive this by splitting the hot user — celebrity sharding, where one
lopsided account is spread across several shards — or by keeping a secondary index.

**Shard by photo ID** instead, and writes spread beautifully: a celebrity's uploads
scatter across every shard, and no single shard sweats. The cost is on the read side —
Maya's photos now live on *all* shards, so "show me her latest 20" fans out across the
fleet in a scatter-gather. Unless you build a secondary user→photos index so the router
knows where her photos live — for the sharp reader who just asked: yes, that index
exists in real systems, and yes, keeping it consistent is a cost of its own.

This isn't hypothetical: Instagram's engineering team has written about running sharded
...
<VideoCard videoId="XP98YCr-iXQ" title="What is Database Sharding?" source="Anton Putra" description="A senior engineer explains sharding like splitting a phone book across offices — same data, smarter routing — and how to pick a shard key so one office doesn&#x27;t drown." />
...
<VideoCard videoId="zEUfss3Jork" title="Amazon S3 Explained Buckets, Objects &amp; Permissions" source="Hired in IT (unverified — confirm channel on YouTube)" description="Teaches object storage like renting storage units — buckets, keys, and who gets a key — including the permission mistakes that bite, plus S3-plus-CloudFront serving." />
...
<VideoCard videoId="3BSC0FfaXYA" title="System Design 101: News Feed Architecture" source="Ptolémé" description="Compares fan-out on write vs read for feed generation, caching, and ranking tradeoffs." />
...
<VideoCard videoId="n6Mts3gomNA" title="Neo4j (Graphdb) vs SQL (RDBMS) | Graph Database Explained for SQL Developers" source="Learn IT Tech" description="Maps SQL tables and JOINs onto graph nodes and relationships so the &quot;why graphs&quot; moment clicks — like swapping a filing cabinet for a whiteboard of connections." />
...
<VideoCard videoId="IUrQ5_g3XKs" title="System Design Interview: Design YouTube w/ a Ex-Meta Staff Engineer" source="Hello Interview" description="An ex-Meta staff engineer whiteboards YouTube end to end — like watching a senior think out loud through every trade-off from upload to playback." />
...
- **Hot shards.** If you shard metadata by user ID, the celebrity's shard becomes a
  ...
  instead — but now one user's profile read fans out across the fleet. Either way, the
  shard key must survive your most lopsided user, and in the interview you should name
  the pain you chose.
...
<VideoCard videoId="M1Xv-sxBG1E" title="Design the News Feed (Twitter / X) — System Design Interview" source="The Hot Path" description="Starts where the real difficulty lives — the celebrity hot-key that breaks naive fan-out — then shows the hybrid and failover fixes, like a post-mortem before the outage." />
...
<VideoCard videoId="Qj4-GruzyDU" title="Design FB News Feed System Design Interview w/ ex: Meta Senior Manager" source="Hello Interview (unverified — confirm channel on YouTube)" description="Full interview-framework walkthrough, requirements to deep dives, with an ex-Meta senior manager — a dress rehearsal for the real round." />
...
- [Designing INSTAGRAM: System Design of News Feed](https://www.youtube.com/watch?v=QmX2NPkJTKg) — Gaurav Sen, YouTube tutorial (~25 min). Schema, microservices, caching, and the celebrity-post fanout problem.
- [Designing Instagram: A Social Media Platform System Design Interview Guide](https://www.youtube.com/watch?v=GMzFBT1_vdQ) — YouTube tutorial. Capacity planning, API design, schema, caching, and sharding.
- [Design Instagram Reels System — Complete Capacity Planning & System Design](https://www.youtube.com/watch?v=qxjyA-BXHxk) — YouTube tutorial. Upload pipeline, object storage, CDN, and feed generation at scale.
- [System Design Mock Interview: Design Instagram](https://www.youtube.com/watch?v=VJpfO6KdyWE) — Aced (formerly Exponent). A full mock-interview walkthrough of this exact lesson problem — requirements, scale estimation, and photo-sharing design — from an established interview-prep platform.
- [Instagram's Architecture, Explained | System Design Breakdown | Episode 01](https://www.youtube.com/watch?v=fRsajShniPk) — The Tech Intern. AI-narrated alternative.
- [System Design Episode 4 || Capacity Estimation & Back-of-the-Envelope Calculations](https://www.youtube.com/watch?v=L3GLkH2oscw) — CodeTav Management. AI-narrated alternative.

...
- **The thundering herd.** A photo goes viral and a million phones request the same
  ...
  Request collapsing at the edge (one origin fetch per PoP per object) is what keeps
...
- Martin Kleppmann, *Designing Data-Intensive Applications* (O'Reilly, 2017), Ch. 5 —
  covers the fan-out vs fan-in trade-off between doing work on write versus on read.

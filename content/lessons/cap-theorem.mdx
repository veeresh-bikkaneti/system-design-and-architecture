...
<VideoCard videoId="KmGy3sU6Xw8" title="My thoughts on the CAP theorem" source="Hussein Nasser" description="Hussein Nasser uses a master-and-replicas story to show how CAP trade-offs shape real architecture decisions." />
...
<VideoCard videoId="BHqjEjzAicA" title="CAP Theorem Simplified" source="ByteByteGo" description="Gaurav Sen explains each guarantee with simple visuals so the three CAP promises are easy to picture and compare." />
...
<VideoCard videoId="xoh6jYqPdek" title="CAP Theorem Explained in 6 Minutes | System Design Interview #11" source="Learn with Manoj" description="Defines consistency, availability, and partition tolerance exactly, then states the real tradeoff clearly." />
...
<VideoCard videoId="IzbYUjPAm5A" title="Why Are Distributed Systems so Hard? • Denise Yu • GOTO 2020" source="GOTO Conferences" description="Conference talk clearing up CAP myths, like why partition tolerance is never optional." />
...
3. CAP's C means **linearizability** — every read sees the latest completed write, as if
   there were only one copy — which is a different idea from ACID's C (transactions
   preserve your data's rules). Same word, different meaning entirely.
  ...
  questions={[
    {
      ...
      options: [
        ...
        "Shut down both diners permanently rather than risk a wrong answer",
    ...
    },
    {
      ...
      options: [
        ...
        "A logical division of a database's rows across its servers",
        "A firewall rule that blocks all client traffic to the cluster",
        "A scheduled maintenance window during off-peak hours",
      ],
      correctIndex: 0,
    },
    {
    ...
    },
    {
      ...
      options: ["ZooKeeper", "Cassandra", "etcd", "HBase"],
      correctIndex: 1,
    },
  ]}
/>
...
- [Are you Using CAP Theorem Wrong?](https://www.youtube.com/watch?v=5XOgPeChuE4) — LearnThatStack, YouTube tutorial (~18 min). Why "pick two" is a myth, plus PACELC and CRDTs.
- [System design from First Principle[8/15]: CAP Theorem & PACELC](https://www.youtube.com/watch?v=Ub9yZucpwC0) — YouTube series. Why partition tolerance isn't a choice; LWW versus vector clocks.
- [System Design Basics #15 — The CAP Theorem: Consistency vs. Availability, Honestly](https://www.youtube.com/watch?v=sw4V1mdfm4M) — YouTube series (~10 min). A network partition walked through CP versus AP, with PACELC.
- [Transactions: myths, surprises and opportunities](https://www.youtube.com/watch?v=5ZjhNTM8XU8) — Martin Kleppmann, Strange Loop 2015 (~41 min). The closest thing to a definitive CAP treatment — the *Designing Data-Intensive Applications* author tackles the "much-misunderstood CAP theorem" head on; the CAP discussion sits in the second half, after ACID and weak-isolation race conditions.

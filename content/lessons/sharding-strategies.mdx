...
<VideoCard videoId="TEWauzwcFco" title="System Design Basics #13 — Sharding and Partitioning: When One Machine Isn't Enough" source="LLMwork" description="Walks through range-based sharding, the shard-by-date hotspot, and spreading fixes." />
...
<VideoCard videoId="qgsYShQhVZo" title="System Design Basics #14 — Consistent Hashing: Adding a Server Without Moving Everything" source="LLMwork" description="Builds the hash ring from the modulo-hashing failure, showing why adding a server moves only ~1/N keys; covers virtual nodes." />

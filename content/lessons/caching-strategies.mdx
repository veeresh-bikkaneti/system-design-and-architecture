...
<VideoCard videoId="TQPUHnuXWcY" title="Why Is Your App Slow? Redis & Caching Explained | C# Cache-Aside" source="Backend Developer" description="A café analogy shows what repeated DB reads cost — and what caching saves." />
...
<VideoCard videoId="hk50ZQuZGgQ" title="How One Cache Turns 200ms Into 1ms — Caching Strategies Explained" source="prahack" description="The 200ms-to-1ms win, then the catch: a second copy drifts out of date." />
...
<VsToggle
  ...
  a={{
    ...
    points: [
      ...
      "Hot keys stay warm while their TTL keeps refreshing",
    ],
...
<VideoCard videoId="CVQz0E33ft4" title="A Step-by-Step Guide for the Cache-Aside Pattern + Stampede Protection" source="Milan Jovanović" description="An established educator walks cache-aside through implementation, pros and cons included." />
...
pair the async memtable → SSTable flush with a durable **commit log** written synchronously
before the write is acknowledged — that pairing is precisely what makes their write-behind
safe, and why they don't lose acknowledged writes on a crash. Application caches rarely have
that safety net, so they should avoid write-behind unless you can afford to lose recent
writes.

<VideoCard videoId="yf1VvSXUANI" title="System Design Part 3: Caching Layer" source="Sketch Mind" description="Dedicated chapters contrast read-through vs write-through, and the freshness each one buys." />

## Eviction: when the pantry is full
...
<VideoCard videoId="vAqFJp_eaGI" title="Ep. 21: Cache Eviction Explained in Detail" source="Systemic Stack English" description="Compares FIFO, LRU, and LFU with workload trade-offs — exactly this subtopic." />
...
<VideoCard videoId="-q_Ku081_SI" title="Hot Keys & Cache Stampedes Explained | Prevent Database Meltdowns | System Design" source="Tejav Academy" description="A dedicated stampede explainer: warming, stale-while-revalidate, and request coalescing." />
...
<VideoCard videoId="ZIDjSV1RZBU" title="Caching at Different Levels: Client, CDN, Redis & DB | System Design" source="InfraWithDipankar" description="Walks the four layers — client, CDN, Redis, database — and when each one goes wrong." />
  ...
  questions={[
    {
      ...
      options: [
        "The cache itself loads the data from the database and returns it",
        "The application loads the data from the database, stores it in the cache, then serves it",
        "The request fails and the client must retry",
        "The application writes a placeholder and serves stale data",
      ],
      correctIndex: 1,
    },
    {
      ...
      options: [
        "Reads become slower because the cache must check the database first",
        "Writes can be lost if the cache fails before flushing to the database",
        "The database receives every write twice, doubling load",
        "Cache hit ratios drop because entries are never evicted",
      ],
      correctIndex: 1,
    },
    {
      ...
      options: [
        "Too many cache nodes join the cluster at once",
        "A popular key expires and many requests simultaneously hit the database with the same query",
        "The eviction policy deletes keys faster than they are written",
        "Two caches in different regions return conflicting values",
      ],
      correctIndex: 1,
    },
    {
      ...
      options: [
        "Increasing the TTL on all keys to the maximum",
        "Disabling the cache during traffic spikes",
        "Request coalescing (single-flight): only the first request queries the database while others wait",
        "Switching from LRU to LFU eviction",
      ],
      correctIndex: 2,
    },
  ]}

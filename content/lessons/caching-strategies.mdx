...
*and* the database synchronously, so the cache is never stale (but writes pay the cost of
both).
...
use this internally (memtable → SSTable); application caches rarely should, unless you can
afford to lose recent writes.
...
- Martin Kleppmann, *Designing Data-Intensive Applications* (O'Reilly, 2017), Ch. 11 — caching in stream processing and derived data.
...
- Facebook Engineering, "Scaling Memcached at Facebook" (NSDI 2013) — cache consistency and thundering-herd mitigation at massive scale.

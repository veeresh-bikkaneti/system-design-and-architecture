...
almost always a cache doing the heavy lifting. Caching is the single highest-leverage
optimization you can apply to a system, and also one of the easiest to get subtly wrong.
...
{/* ANIMATION: RequestFlow showing a cache-aside read path — cache hit vs cache miss with the database round-trip highlighted */}
...
*and* the database synchronously, so the cache stays fresh on the write path — though concurrent
reads racing a write, or a partial failure between the two writes, can still surface briefly
stale data (but writes pay the cost of both).
...
immediately; the database is updated asynchronously. Writes get blazing fast, but if the
...
Caching isn't one layer — it's a stack. Browser caches, **CDN** edge caches (next lesson!),

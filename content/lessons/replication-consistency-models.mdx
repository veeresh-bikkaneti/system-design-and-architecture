export const meta = {
  ...
  estimatedMinutes: 20,
  difficulty: 4,
  topics: ['databases', 'consistency', 'distributed systems'],
};
...
<VideoCard videoId="mBUCF1WGI_I" title="Distributed Systems 5.1: Replication" source="Martin Kleppmann" description="Kleppmann shows why copies of data disagree — idempotent retries, tombstones, and anti-entropy, like roommates comparing shopping lists." />
...
- **Synchronous** — the write isn't acknowledged until replicas confirm. Durable (a leader
  ...
  **Postgres** synchronous streaming replication and **etcd**'s Raft log work this way.
  (etcd is the key-value store Kubernetes keeps its cluster state in; its Raft log is the
  ordered list of changes every node replays — you'll take Raft apart in the advanced
  consensus-raft lesson.)
...
<VideoCard videoId="fsZA4K0E3kQ" title="Database Replication Explained (Replication Lag, Sync vs Async) | System Design with Microsoft SWE" source="Mayank Joshi" description="Covers synchronous versus asynchronous replication, lag, and their real trade-offs." />
...
<PacketFlow
  stages={["Client", "Coordinator node"]}
  servers={["Replica A", "Replica B", "Replica C"]}
  requestLabel="W"
  duration={5}
/>

<VideoCard videoId="RK2odztfP0I" title="L27: Leaderless Replication, Topologies, CRDTs, Quorum Writes, Read Repair &amp; Anti-Entropy" source="Aarchi Gandhi" description="A friendly lecture on life without a leader — gossip, quorum math, and read repair, like a group chat with no admin." />
...
<StepThrough
  title="Quorum in action: N=3, W=2, R=2"
  nodes={[
    { id: "client", label: "Client", x: 70, y: 150 },
    { id: "coord", label: "Coordinator", x: 200, y: 150 },
    { id: "n1", label: "N1", x: 360, y: 60 },
    { id: "n2", label: "N2", x: 360, y: 150 },
    { id: "n3", label: "N3", x: 360, y: 240 },
  ]}
  edges={[
    { from: "client", to: "coord" },
    { from: "coord", to: "n1" },
    { from: "coord", to: "n2" },
    { from: "coord", to: "n3" },
  ]}
  steps={[
    {
      title: "Write fans out to all three",
      caption: "The coordinator sends the write to N1, N2, and N3. It doesn't need all three to answer — just W of them.",
      highlight: ["client", "coord", "n1", "n2", "n3"],
    },
    {
      title: "W=2 acknowledges",
      caption: "N1 and N2 ack quickly; N3 is slow. Two acks make a quorum, so the write succeeds — N3 catches up in the background.",
      highlight: ["n1", "n2"],
    },
    {
      title: "Read asks two nodes",
      caption: "R=2: the read goes to N2 and N3 and takes the newest value it finds. It doesn't matter that N3 missed the write.",
      highlight: ["client", "n2", "n3"],
    },
    {
      title: "Overlap guarantees freshness",
      caption: "N2 sat in both the write quorum and the read quorum, so the read sees the latest write. R + W = 4 > N = 3: every read overlaps every write on at least one node.",
      highlight: ["n2"],
    },
  ]}
/>
...
<VideoCard videoId="uNxl3BFcKSA" title="Distributed Systems 5.2: Quorums" source="Martin Kleppmann" description="Explains W+R>N quorum tuning with latency trade-offs in a compact, focused format." />
...
- **Causal consistency** — if you reply to a comment, everyone sees the comment before
  ...
  (COPS is a 2011 research system from Cornell — still the canonical example of causal
  consistency running across data centers.)
...
<VideoCard videoId="noUNH3jDLC0" title="Distributed Systems 7.2: Linearizability" source="Martin Kleppmann" description="Walks the full spectrum from strong to eventual consistency — when you need bank-level agreement and when gossip is good enough, like certified mail versus a rumor." />
...
- **Application merge** — the app gets both versions and reconciles. That reconciliation
  runs on Dynamo/Riak-style vector clocks — per-replica version stamps that record which
  writes saw which other writes — and on CRDTs for counters, sets, and text: data
  structures designed so concurrent edits always merge to the same answer on every
  replica, no referee needed.
...
<VideoCard videoId="l3WgzxkYNjI" title="How CRDTs Actually Work — The Data Structures Behind Real-Time Collaborative Apps" source="State & Flow" description="Shows how CRDTs resolve conflicts in real-time collaborative apps, no coordination needed." />
...
- [Call Me Maybe: Distributed Databases and Linearizability](https://www.youtube.com/watch?v=I6fD3FhwDio) — Kyle Kingsbury, Philly ETE 2014 (~57 min). What "strongly consistent" really means under partitions, via the Jepsen project.
- [CRDTs: The Hard Parts](https://www.youtube.com/watch?v=x7drE24geUw) — Martin Kleppmann, Hydra 2020. Conflict-free replicated data types past the intro: resolving concurrent writes on replicas.
- [Dynamo - Amazon's Highly Available Key-Value Store](https://www.youtube.com/watch?v=hLWgwzzhY0c) — YouTube paper explainer. The Dynamo paper walked through: quorums, vector clocks, gossip, hinted handoff.
- [System Design Concepts Course and Interview Prep](https://www.youtube.com/watch?v=F2FmTdLtb_4) — freeCodeCamp.org (~54m). Replication and ACID chapters in one sitting — an interview-grade refresher on what each consistency level actually buys you.
- [Distributed Consensus and Data Replication strategies on the server](https://www.youtube.com/watch?v=GeGxgmPTe4c) — Gaurav Sen. Sync vs async replication, split brain, and quorum — the machinery behind every consistency model in this lesson.
- [SD 6 | Consistency Models: Strong vs Eventual | System Design Course | Uplatz](https://www.youtube.com/watch?v=2qkVSJ9QbQA) — Uplatz. AI-narrated alternative.

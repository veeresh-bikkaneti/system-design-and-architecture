...
- **Causal consistency** — if you reply to a comment, everyone sees the comment before
  your reply. The strongest model that's still available during partitions, and what
  systems like **CockroachDB** aim to give you cheaply.
...
- **Application merge** — the app gets both versions and reconciles (DynamoDB-style
  version vectors; CRDTs for counters, sets, and text).
...
- CockroachDB documentation, "Consistency" — causal consistency and closed timestamps in a modern distributed SQL database.

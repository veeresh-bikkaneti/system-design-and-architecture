...
//
// Reads content/lessons/*.mdx, chunks each lesson, and emits two artifacts:
//
//   1. worker/src/qa/qa-index.ts -- bundled keyword index: per-term top-N
//      BM25 postings (integer scores) + chunk metadata/excerpts. This is what
//      the Worker ranks against at request time. No D1 round-trip needed.
//
//   2. worker/migrations/0006_qa_chunks.sql -- full chunk texts in D1
//      (qa_chunks table) for P2's read_lesson tool. Apply with wrangler
//      alongside 0004_rate_limits.sql and 0005_qa_sessions.sql.
//
...
/** Pure build: lessons in, chunks + bundled index + migration SQL out. */
export function buildIndexFromLessons(lessons: LessonInput[], generatedAt: string): BuiltIndex {
  ...
  const lines: string[] = [
    ...
    '-- Full lesson chunk texts for the course Q&A agent (P1 retrieval).',
    '-- Quiz blocks are stripped before chunking: no answer keys in this file.',
    '--',
    `-- Apply alongside 0004_rate_limits.sql and 0005_qa_sessions.sql, e.g.:`,
    ...
    '  text TEXT NOT NULL',
    ');',
    'CREATE INDEX IF NOT EXISTS idx_qa_chunks_slug ON qa_chunks(slug);',
    '',
  ];
  for (const c of chunks) {
    lines.push(
      `INSERT INTO qa_chunks (id, slug, title, heading, ordinal, text) VALUES (${c.id}, ${sqlEscape(c.slug)}, ${sqlEscape(c.title)}, ${sqlEscape(c.heading)}, ${c.ordinal}, ${sqlEscape(c.text)});`,
    );
  }
  // eslint-disable-next-line no-console
  if (dropped > 0) console.log(`qa:index: dropped ${dropped} terms over the byte budget`);

  return { chunks, indexData, migrationSql: lines.join('\n') + '\n' };
}

function parseMeta(mdx: string, file: string): { slug: string; title: string } {
  const slug = /^export const meta = \{[^}]*?slug:\s*(['"])(.*?)\1/m.exec(mdx)?.[2];
  const title = /^export const meta = \{[^}]*?title:\s*(['"])(.*?)\1/m.exec(mdx)?.[2];
  if (!slug || !title) throw new Error(`qa:index: cannot parse meta in ${file}`);
  return { slug, title };
}
...
function main(): void {
  ...
  const lessons: LessonInput[] = files.map((file) => {
    const mdx = readFileSync(join(lessonsDir, file), 'utf8');
    const { slug, title } = parseMeta(mdx, file);
    return { slug, title, mdx };
  });

  const generatedAt = new Date().toISOString();
  const { chunks, indexData, migrationSql } = buildIndexFromLessons(lessons, generatedAt);
  ...
  const migrationPath = join(migrationsDir, '0006_qa_chunks.sql');
  ...
  console.log(
    `qa:index: ${lessons.length} lessons -> ${chunks.length} chunks (${words} words), ` +
      ...
      `index ${(indexBytes / 1024).toFixed(0)}KB, migration ${(Buffer.byteLength(migrationSql) / 1024).toFixed(0)}KB`,
  );
}

...
/**
 * Anonymous Session architecture: the widget's entire memory is whatever
 * useQaHistory persists to localStorage -- there is no server-side session.
 * These tests exercise that persistence layer directly (no DOM needed).
 */
describe('useQaHistory persistence', () => {
  ...
  it('round-trips a stored transcript', () => {
    const messages: StoredMessage[] = [
      ...
      { id: 1, role: 'assistant', content: 'FINAL: the course covers this.' },
    ];

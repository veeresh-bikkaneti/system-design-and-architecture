...
/**
 * There is no server-side session of any kind: the browser IS the memory.
 * This hook persists the widget's message transcript to localStorage so it
 * survives a reload -- the on-device model (../../ai/local/inference.ts) is
 * called fresh on every turn, and this array is its only continuity.
 */
